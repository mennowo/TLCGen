...
/* werner 20130222 : duidelijke scheiding gemaakt */
/* tussen de array en de typedefinitie.           */
...
void OVInit(void) {
  ...
  for (ov1=0;
    ...
    for (ov2=0;
         ...
         ov2++) {
      fc2=iFC_OVix[ov2];
      if (TO_max[fc1][fc2]==GK || TO_max[fc1][fc2]==GKL) { /* toegevoegd Ane 25-04-2011, GKL */
...
/* ---------------------------------------------------------------------------------
   OV Scenario bepaalt het rijtijdscenario voor een OV-richting (ov)
...
void AfkappenMG(int fc, int iStartGr) {
  ...
  for (i=0;
    ...
    if (MG[k] &&
        (TO_max[k][fc]>= 0 && (TGL_max[k]>0 ? TGL_max[k] : 1) + TO_max[k][fc] >= iStartGr||
         TO_max[k][fc]==GK && iStartGr<=0) ||
		 TO_max[k][fc]==GKL && TGK_max[k][fc] >= iStartGr) {							/* TO_max[k][fc]==GKL toegevoegd */
      Z[k]|=OV_Z_BIT;
    }
  }
}
...
void OVAlternatieven(void) {
  ...
  for (fc=0; fc<FCMAX; fc++) {
    ...
    RR[fc] &= ~BIT4;
		...
		/* Rotterdam - Modificatie 2					 */	
		/* Aanpassing: alternatieve realisaties toestaan */
		/* vóór startgroen moment van OV richting        */
		/* 02-04-2009 / William Meijer					 */
  for (fc = 0; fc < FCMAX; fc++) {
