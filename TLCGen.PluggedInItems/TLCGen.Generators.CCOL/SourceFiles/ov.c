...

{
    ...
    for (fc = 0; !iMaximumWachtTijdOverschreden && fc < FCMAX; ++fc)
    {
...
void PrioriteitsToekenning(void)
{
    ...
    for (ov = 0; ov < ovOVMAX; ++ov)
    {
        ...
        if (BL[fc] ||
            iAantalPrioriteitsInmeldingen[ov] == 0 ||
            !G[fc] && iMaximumWachtTijdOverschreden && !(iPrioriteitsOpties[ov] & poNoodDienst))
        {
            iPrioriteit[ov] = 0;
        }
        iKOVPrio[ov] = -1;
...
#if defined CCOLTIG && !defined NO_TIGMAX
            (TIG_max[k][fc] >= 0 && (TGL_max[k] > 0 ? TGL_max[k] : 1) + TIG_max[k][fc] >= iStartGr ||
             TIG_max[k][fc] == GK && iStartGr <= 0) || TIG_max[k][fc] == GKL) /* TIG_max[k][fc]==GKL toegevoegd */
#else
            (TO_max[k][fc] >= 0 && (TGL_max[k] > 0 ? TGL_max[k] : 1) + TO_max[k][fc] >= iStartGr ||
...
void OVAlternatieven(void)
{
...
#if defined CCOLTIG && !defined NO_TIGMAX
                        IH[iH_OVix[ov]] && R[iFC_OVix[ov]] && (TIG_max[fc][iFC_OVix[ov]] == NG) && (iPrioriteitsOpties[ov] & poBijzonderRealiseren)) ? OV_PAR_BIT : 0;
#else
    ...
    if (iLWAlt)
    {
        ...
        for (fc = 0; fc < FCMAX; ++fc)
        {
            /* Alternatieve toedeling ook zetten bij alternatieve realisatie */
