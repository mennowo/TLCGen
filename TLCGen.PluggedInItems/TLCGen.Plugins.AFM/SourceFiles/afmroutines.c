 ...
 * 6	18-07-2017  psn		Kleine wijziging
 ************************************************************************************/
...
void AFMinterface(AFM_FC_STRUCT * AFM_data_fc)
{
	...
	{
		if (PRM[afmprm + AFM_GMAXCCOL] != AFM_data_fc->gmax_ccol) 
		{
			PRM[afmprm + AFM_GMAXCCOL] = AFM_data_fc->gmax_ccol;
			AFM_CIF_changed=TRUE;
		}
		if (PRM[afmprm + AFM_GMAXACT] != AFM_data_fc->gact)
		{
			PRM[afmprm + AFM_GMAXACT] = AFM_data_fc->gact;
			AFM_CIF_changed=TRUE;
		}
		if (PRM[afmprm + AFM_GMAXGEM] != AFM_data_fc->gmax_gem)
		{
			PRM[afmprm + AFM_GMAXGEM] = AFM_data_fc->gmax_gem;
			AFM_CIF_changed=TRUE;
		}
		if (PRM[afmprm + AFM_AFGEKAPT] != AFM_data_fc->afgekapt)
		{
...
void AFMacties(AFM_FC_STRUCT * AFM_data_fc, mulv fc_shadow, AFM_FC_STRUCT AFM_data_fcs[AFM_fcmax])
{
	...
	if ((AFM_data_fc->gmax_afm  < AFM_data_fc->gmax_ccol) && (AFM_data_fc->sturing_afm!=0) && T[tAFMLeven])
	{
	...
	if ((AFM_data_fc->gmax_afm  > AFM_data_fc->gmax_ccol) && (AFM_data_fc->sturing_afm!=0))
	{
	...
	if (!(fc_shadow==NG))
	{
		...
		if (G[fc_shadow] && (AFM_data_fc->gmax_afm  < AFM_data_fc->gmax_ccol) && PRM[prmAFM_Strikt] && (AFM_data_fc->sturing_afm!=0))
		{
			MK[fc_shadow]|= BIT13;
		}
	}
	...
	if (PRM[prmAFM_Strikt] && (AFM_data_fc->sturing_afm  == 0))
	{
		DeltaSec = 0;
				...
				if ((PRM[afmprm + AFM_GMAXAFM] > PRM[afmprm + AFM_GMAXCCOL]) && PRM[afmprm + AFM_STURING])
				{
					if (MLMAX>1)
					{
						...
						else
						{
							...
							{
								DeltaSec = (PRM[afmprm + AFM_GMAXMAX] - PRM[afmprm + AFM_GMAXCCOL]) / (MLMAX - 1);
							}
						}
...
int AFMmonitor_data_pre(int x, int y)
{
	...
	xyprintf(x, yret++, "  PR fc  GMAXAFM   TVG   TVG_g  Afgekapt");
...
void AFMinterfacemonitor(AFM_FC_STRUCT afmfcstructs[AFM_fcmax])
{
	...
	xyprintf(2, 10+AFM_fcmax, "PRM[prmAFM_tc] = %2d  PRM[prmAFM_tcgem] = %2d", PRM[prmAFM_TC], PRM[prmAFM_TCgem]);
