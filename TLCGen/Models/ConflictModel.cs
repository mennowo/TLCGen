        ...
        public string SerializedWaarde
        {
            ...
            set
            {
                switch (value)
                {
                    ...
                    default:
                        int waarde;
                        if (!Int32.TryParse(SerializedWaarde, out waarde))
                        {
                            MessageBox.Show($"Fout bij laden conflicten:\n\nconflicten van {FaseVan} naar {FaseNaar} heeft foutieve waarde {SerializedWaarde}", "Fout bij laden conflicten.");
                        }
                        else
                        {
                            Waarde = waarde;
